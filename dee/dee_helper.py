    ...
    def get_entity_label_list():
        ...
        for field in common_fields:
            if field not in visit_set:
                visit_set.add(field)
                entity_label_list.extend(['B-' + field, 'I-' + field])  # append列表
        # fieldName contained in event
...
class DEEFeature(object):
    def __init__(self, guid, ex_idx, doc_token_id_mat, doc_token_mask_mat, doc_token_label_mat,
        ...
        # [(token_id, ...), ...]
        # span_idx -> span_token_id tuple, token is a character
    ...
    def generate_dag_info_for(self, pred_span_token_tup_list, return_miss=False):
        ...
        for gold_span_idx, token_tup in enumerate(self.span_token_ids_list):  # every span's token
            if token_tup in token_tup2pred_span_idx:
                pred_span_idx = token_tup2pred_span_idx[token_tup]  # token's span
                gold_span_idx2pred_span_idx[gold_span_idx] = pred_span_idx  # gold span_id is all span idx include span not to be role
                # pred_span_idx2gold_span_idx[pred_span_idx] = gold_span_idx
            else:
                missed_span_idx_list.append(gold_span_idx)  # this span was missed
        ...
        for event_arg_idxs_objs in self.event_arg_idxs_objs_list:
            # event_arg_idxs_objs, document's events
    ...
    def build_dag_info(event_arg_idxs_objs_list):
        ...
        for event_idx, event_arg_idxs_list in enumerate(event_arg_idxs_objs_list):
            if event_arg_idxs_list is None:
                ...
                num_fields = len(event_arg_idxs_list[0])  # num_fields of this event
                ...
                for field_idx in range(num_fields):
                    ...
                    for event_arg_idxs in event_arg_idxs_list:
                        # args in one event
                        pre_path = event_arg_idxs[:field_idx]  # list, means a[0]->a[1]->a[2]
                    ...
                    field_idx2pre_path2cur_span_idx_set.append(pre_path2cur_span_idx_set)  # all path set
                    # 需要枚举所有路径，这里的计算复杂度是很大的
...
class DEEFeatureConverter(object):
    ...
    def convert_example_to_feature(self, ex_idx, dee_example, log_flag=False):
        ...
        # doc_token_id，doc_token_mask，doc_token_label Size(num_sent_num, num_sent_len)
        doc_token_id_mat = []  # [[token_idx, ...], ...]
        doc_token_mask_mat = []  # [[token_mask, ...], ...]
        doc_token_label_mat = []  # [[token_label_id, ...], ...]

        for sent_idx, sent_text in enumerate(dee_example.sentences):  # 遍历sentence
        ...
        # 2. prepare span feature， relation between span and token
        # span_token_ids_list, span_dranges_list
...
def resume_decode_results(base_dir, data_type, span_type, model_str, epoch):
    ...
    decode_results = default_load_pkl(decode_fp)  # load for decoder
...
def print_single_vs_multi_performance(mstr_bepoch_list, base_dir, features,
    ...
    for model_str, best_epoch in mstr_bepoch_list:  # load model
