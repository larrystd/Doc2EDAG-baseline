...
class DEETaskSetting(TaskSetting):
    ...
    base_attr_default_pairs = [
        ...
        ('bert_model', './bert_pretrain'),  # use which pretrained bert model
...
class DEETask(BasePytorchTask):
    ...
    def __init__(self, dee_setting, load_train=True, load_dev=True, load_test=True,
        ...
        # load vocabulary
        self.tokenizer = BERTChineseCharacterTokenizer.from_pretrained(self.setting.bert_model + '/bert-base-chinese-vocab.txt')
        self.setting.vocab_size = len(self.tokenizer.vocab)

        # get entity and event label name
        self.entity_label_list = DEEExample.get_entity_label_list()  # event label, type:string
        self.event_type_fields_pairs = DEEExample.get_event_type_fields_pairs()  # event name, event field

        # build example loader
        self.example_loader_func = DEEExampleLoader(self.setting.rearrange_sent, self.setting.max_sent_len)

        # build feature converter, convert example to feature
        ...
        # load data, based on example_loader_func, example->feature->dataset, save in self.train_examples, self.train_features, self.train_dataset
        self._load_data(
        ...
        self.min_teacher_prob = None  # knowlege distillation
