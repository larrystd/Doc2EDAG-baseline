...
def load_ner_dataset(dataset_json_path):
    total_ner_examples = []
    annguid2detail_align_info = default_load_json(dataset_json_path)  # load json
    """key-value
    "sentences": 句子
    "ann_valid_mspans": 有效spans
    "ann_valid_dranges": 有效span的范围
    "ann_mspan2guess_field": span的类型
    """
    # guid, 如SH600641_2009-01-05_47975101
    for annguid, detail_align_info in annguid2detail_align_info.items():
        ...
        for drange, mspan in zip(ann_valid_dranges, ann_valid_mspans):  # zip, 将两个list整成按索引的元组遍历
            sent_idx, char_s, char_e = drange  # span所在的sentence id, 起始和结束位置
        ...
        for sent_idx in range(len(sents)):
            sent_text = sents[sent_idx]  # 每个sentence
