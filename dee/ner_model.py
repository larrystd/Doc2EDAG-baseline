...
class NERModel(nn.Module):
    def __init__(self, config):
        ...
        if self.config.use_crf_layer:
            self.crf_layer = CRFLayer(config.hidden_size, self.config.num_entity_labels)  # CRF
        else:
    ...
    def forward(self, input_ids, input_masks,
        ...
        batch_seq_enc = self.token_encoder(input_emb, input_masks)  # token encoder
        ...
        if self.config.use_crf_layer:
            ...
            if train_flag:
                ...
                batch_label = label_ids.view(-1)
                # ner_loss = F.nll_loss(batch_logp, batch_label, reduction='sum')
                ner_loss = F.nll_loss(batch_logp, batch_label, reduction='none')  # ner loss
                ner_loss = ner_loss.view(label_ids.size()).sum(dim=-1)  # [batch_size]
            else:
                ner_loss = None
...
class CRFLayer(nn.Module):
    ...
    def get_log_parition(self, seq_emit_score):
        ...
        for token_idx in range(seq_len):  # for every token
            ...
            dp_table = log_sum_exp(cur_logit)  # [batch_size, tag_size], maxium likehold, make max tag far bigger than other tag
    ...
    def get_gold_score(self, seq_emit_score, seq_token_label):
        ...
        :param seq_emit_score also hidden: [seq_len, batch_size, tag_size]
        ...
        ).unsqueeze(-1).unsqueeze(-1).expand(seq_len+1, batch_size, 1, tag_size)  # 1*tag_size is label of token
        ...
        # trans_score gain from trans also trans_mat (label tag)
        seq_trans_score = self.trans_mat.unsqueeze(0).unsqueeze(0).expand(seq_len+1, batch_size, tag_size, tag_size)  # used to gather tag_size*tag_size
        # gather according to token label at the current token
        gold_trans_score = torch.gather(seq_trans_score, 2, seq_cur_label)  # [seq_len+1, batch_size, 1, tag_size] from self.trans_mat
        ...
        batch_trans_score = gold_trans_score.sum(dim=0).squeeze(-1).squeeze(-1)  # [batch_size], -1 means the last dim
        # emit_score gain from hidden
    ...
    def viterbi_decode(self, seq_emit_score):
        ...
        for token_idx in range(seq_len):  # for each token seq
            ...
            dp_table, cur_tag_bp = cur_trans_score.max(dim=-1)  # [batch_size, tag_size] update dp_table， cur_tag: (max, max_indices)
        ...
        best_tag = best_tag.unsqueeze(-1)  # [batch_size, 1], tag also called candidate label index
...
def log_sum_exp(batch_logit):
    ...
    batch_broadcast = batch_max.unsqueeze(-1)  # batch_max
    return batch_max + \
